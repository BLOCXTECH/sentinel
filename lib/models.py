...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, blocxd):
        golist = blocxd.rpc_command("gobject", "list")
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_blocxd(blocxd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_blocxd(self, blocxd, rec):
        import decimal
        import blocxlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from blocxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from blocxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from blocxd - BlocXd is the master
        try:
            newdikt = subdikt.copy()
            newdikt["object_hash"] = object_hash
            if subclass(**newdikt).is_valid(blocxd) is False:
                govobj.vote_delete(blocxd)
                return (govobj, None)
            ...
            printdbg("Got invalid object from blocxd! %s" % e)
            govobj.vote_delete(blocxd)
            return (govobj, None)
    ...
    def vote_delete(self, blocxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(blocxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, blocxd, signal, outcome):
        import blocxlib
        ...
        output = blocxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = blocxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(blocxd, signal)

    def sync_network_vote(self, blocxd, signal):
        printdbg(
            "\tSyncing network vote for object %s with signal %s"
            % (self.object_hash, signal.name)
        )
        vote_info = blocxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (blocxlib.blocks_to_seconds(superblockcycle) / 2) + SUPERBLOCK_FUDGE_WINDOW
        )
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self, blocxd=None):
        ...
        network = "mainnet"
        if blocxd is not None:
            network = blocxd.network()

        # it's a string from the DB...
        addresses = self.payment_addresses.split("|")
        for addr in addresses:
            if not blocxlib.is_valid_blocx_address(addr, network):
    ...
    def hash(self):
        ...
        return blocxlib.hashit(self.serialise())

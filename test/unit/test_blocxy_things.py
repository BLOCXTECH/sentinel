...
def valid_blocx_address(network="mainnet"):
    return (
...
def invalid_blocx_address(network="mainnet"):
    return (
...
def test_valid_blocx_address():
    from blocxlib import is_valid_blocx_address

    main = valid_blocx_address()
    test = valid_blocx_address("testnet")

    assert is_valid_blocx_address(main) is True
    assert is_valid_blocx_address(main, "mainnet") is True
    assert is_valid_blocx_address(main, "testnet") is False

    assert is_valid_blocx_address(test) is False
    assert is_valid_blocx_address(test, "mainnet") is False
    assert is_valid_blocx_address(test, "testnet") is True


def test_invalid_blocx_address():
    from blocxlib import is_valid_blocx_address

    main = invalid_blocx_address()
    test = invalid_blocx_address("testnet")

    assert is_valid_blocx_address(main) is False
    assert is_valid_blocx_address(main, "mainnet") is False
    assert is_valid_blocx_address(main, "testnet") is False

    assert is_valid_blocx_address(test) is False
    assert is_valid_blocx_address(test, "mainnet") is False
    assert is_valid_blocx_address(test, "testnet") is False
...
def test_hash_function():
    ...
    hex_hash = "%x" % blocxlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(blocxlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(blocxlib.blocks_to_seconds(2)).quantize(precision) == Decimal(
        314.4
    ).quantize(precision)
    assert int(blocxlib.blocks_to_seconds(16616)) == 2612035
...
def test_parse_raw_votes():
    ...
    votes = blocxlib.parse_raw_votes(sample_raw_votes)
    ...
    votes = blocxlib.parse_raw_votes(sample_raw_votes_weights)
    # sort vote dicts to ensure ordering is the same as the expected output
    # below
    votes = sorted(votes, key=lambda x: x["mn_collateral_outpoint"])
    assert votes == expected

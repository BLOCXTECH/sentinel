...
if [ ! -f /sentinel/blocx.conf ]; then
  if [ -z "$RPCUSER" -o -z "$RPCPASSWORD" -o -z "$RPCPORT" ]; then
    echo "When no /sentinel/blocx.conf is present, you must at least set RPCUSER, RPCPORT and RPCPASSWORD environment variables"
    exit 1
  fi

  echo "" > /sentinel/blocx.conf
  if [ -n "$RPCUSER" ]; then
    echo "rpcuser=${RPCUSER}" >> /sentinel/blocx.conf
  fi
  if [ -n "$RPCPASSWORD" ]; then
    echo "rpcpassword=${RPCPASSWORD}" >> /sentinel/blocx.conf
  fi
  if [ -n "$RPCPORT" ]; then
    echo "rpcport=${RPCPORT}" >> /sentinel/blocx.conf
  fi
fi
...
if [ ! -f /sentinel/sentinel.conf ]; then
  ...
  echo "blocx_conf=/sentinel/blocx.conf" > /sentinel/sentinel.conf
  if [ -n "$RPCHOST" ]; then
